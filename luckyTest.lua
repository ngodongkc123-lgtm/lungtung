...
local GetPinataUID = function()
    ...
    if PinataUid then
        ...
        if Entry and Entry.id == "Mini Lucky Block" then return PinataUid end
        PinataUid = nil
    end
    for uid, v in pairs(Misc) do
        if v.id == "Mini Lucky Block" then PinataUid = uid return uid end
    end
    return nil
end
    ...
    if GetPinataUID() then
        for _, v in pairs(Areas) do
            ...
            if not a and msg ~= "There is already something in this area!" or msg ~= "There are too many random events already in the world!" then 
                repeat a,e = Network.Invoke("MiniLuckyBlock_Consume", GetPinataUID()) task.wait(0.1) until a
            end
            Network.Invoke("MiniLuckyBlock_Consume", GetPinataUID())
        end
    else
        print("No lucky block") break
    end
end
